...
def my_main(input_folder, output_file):
    ...
    for i in input:
        ...
        for line in f:
            ...
            start_s_name = parameters[4]
            end_s_name = parameters[8]
            if(start_s_name not in from_station):
                from_station[start_s_name] = 1
                to_station[start_s_name] = 0
            else:
                from_station[start_s_name] = from_station[start_s_name]+1

            if(end_s_name not in to_station):
                to_station[end_s_name] = 1
                from_station[end_s_name] = 0
            else:
                to_station[end_s_name]=to_station[end_s_name]+1
    ...
    l = list(from_station.keys())
    l.sort()
    for station in l:
        # out[station].append
        s = station + "\t(" + str(from_station[station]) + "," + str(to_station[station]) + ")\n"
        out_f.write(s)
    out_f.close()
