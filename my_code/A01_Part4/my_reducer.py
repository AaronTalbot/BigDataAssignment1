...
def process_line(line):
    ...
    # 1.2. We ouptut the num_in from the line
    num_in = None

    # 1.3. We ouptut the num_out from the line
    num_out = None
    ...
    num_in = int(aux_content[0][1:])
    num_out = int(aux_content[1][:-1])

    # 3. We assign res
    res = (letter, num_in, num_out)
...
def my_reduce(my_input_stream, my_output_stream, my_reducer_input_parameters):
    ...
    for line in my_input_stream:
        ...
        if parameters[0] not in dict_in:
            dict_in[parameters[0]] = parameters[1]
            dict_out[parameters[0]] = parameters[2]
        else:
            dict_in[parameters[0]] += parameters[1]
            dict_out[parameters[0]] += parameters[2]
    l = list(dict_in.keys())
    l.sort()
    for station in l:
        my_output_stream.write(station + "\t" + "(" + str(dict_in[station]) + ", " + str(dict_out[station]) + ")\n")
